...
def _patch_sklearn_privates():
    try:
        ...
        if not hasattr(_sku, "_print_elapsed_time"):
            from contextlib import contextmanager
            @contextmanager
            def _print_elapsed_time(prefix=None): yield
            setattr(_sku, "_print_elapsed_time", _print_elapsed_time)
...
with st.sidebar.expander("🐞 Versiones en runtime", expanded=False):
    import sys
    try:
        import sklearn, imblearn, xgboost, numpy, pandas
        st.write({
            "python": sys.version.split()[0],
            "sklearn": sklearn.__version__,
            "imblearn": getattr(imblearn, "__version__", "?"),
            "xgboost": xgboost.__version__,
            "numpy": numpy.__version__,
            "pandas": pandas.__version__,
        })
    except Exception as _e:
        st.write("No se pudieron leer versiones:", str(_e))
...
with st.expander("🔎 Columnas esperadas por el preprocesador", expanded=False):
    if expected_cols: st.write(expected_cols)
    else: st.info("No se detectaron automáticamente las columnas esperadas.")

def get_preproc_info(preproc):
    input_cols, cat_cols, cat_choices, num_cols = None, [], {}, []
    if preproc is None: return input_cols, cat_cols, cat_choices, num_cols
    try:
        if hasattr(preproc, "feature_names_in_"):
            input_cols = list(preproc.feature_names_in_)
        transformers = getattr(preproc, "transformers_", [])
        from sklearn.pipeline import Pipeline
        try: from sklearn.preprocessing import OneHotEncoder
        except Exception: OneHotEncoder = None
                ...
                if isinstance(transformer, Pipeline):
                    for _, step in transformer.steps:
                        if OneHotEncoder is not None and isinstance(step, OneHotEncoder):
                            ohe = step; break
                        if hasattr(step, "categories_"): ohe = step; break
                else:
                    if hasattr(transformer, "categories_"): ohe = transformer
...
def predict_model_03(df: pd.DataFrame):
    ...
    proba = None
    if hasattr(model, "predict_proba"):
        try: proba = model.predict_proba(df)
        except Exception: proba = None
    return labels, proba

def sanitize_for_model(df: pd.DataFrame, expected_cols: list|None, cat_cols_all: list|None, excluded: set|None=None, num_cols_all: list|None=None) -> pd.DataFrame:
    import pandas as _pd, numpy as _np
    def _is_missing(v):
        try: return _pd.isna(v)
    ...
    if expected_cols is not None:
        for c in expected_cols:
            if c not in dfx.columns: dfx[c] = _np.nan
        for c in excluded:
            if c in expected_cols: dfx[c] = _np.nan
        dfx = dfx[expected_cols]
    for c in dfx.columns:
        if c in excluded: continue
        if c in cat_set:
            dfx[c] = dfx[c].astype("object")
            dfx[c] = dfx[c].apply(lambda v: _np.nan if _is_missing(v) or (isinstance(v,str) and v.strip()=="") else str(v))
        else:
